...
def test_callback_without_repo_path(mock_commits_in_period):
    ...
    mock_commits_in_period.side_effect = ValueError("No repository path provided. Please run the application with a repository path as a command-line argument.")
    ...
    try:
        ...
        if hasattr(figure, 'layout') and hasattr(figure.layout, 'annotations') and figure.layout.annotations:
            ...
            assert "No repository path provided" in message, "Figure should contain message about missing repository path"
