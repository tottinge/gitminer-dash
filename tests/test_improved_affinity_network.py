...
def compare_networks(period, min_affinity_original=0.5, min_affinity_improved=0.2):
    ...
    print(f"Original min_affinity: {min_affinity_original}, Improved min_affinity: {min_affinity_improved}")
    ...
    # Print comparison
    print("\nComparison Results:")
    print(f"Original Network: {comparison['original']['nodes']} nodes, {comparison['original']['edges']} edges, " +
          f"{comparison['original']['communities']} communities, {comparison['original']['processing_time']:.2f} seconds")
    print(f"Improved Network: {comparison['improved']['nodes']} nodes, {comparison['improved']['edges']} edges, " +
          f"{comparison['improved']['communities']} communities, {comparison['improved']['processing_time']:.2f} seconds")
    
    # Save visualizations
    save_comparison_visualizations(G_original, communities_original, G_improved, communities_improved, period)
    
    return comparison

def save_comparison_visualizations(G_original, communities_original, G_improved, communities_improved, period):
    """
    ...
    fig_improved = create_improved_network_visualization(
        G_improved, communities_improved, 
...
def main():
    ...
    for comparison in all_comparisons:
        ...
        edge_percent = (edge_increase / original['edges'] * 100) if original['edges'] > 0 else float('inf')
