...
def test_calculate_file_commit_frequency_empty_commits(mock_repo):
    ...
    with patch(
        ...
        result = calculate_file_commit_frequency([], mock_repo, begin, end, top_n=20)
...
def test_calculate_file_commit_frequency_rounding(mock_commits_with_files, mock_repo):
    ...
    with patch(
        "algorithms.file_changes.files_changes_over_period", return_value=file_stats
    ):
        result = calculate_file_commit_frequency(
            mock_commits_with_files, mock_repo, begin, end, top_n=1
        )
...
def test_calculate_file_commit_frequency_value_error_propagates(mock_repo):
    ...
    with pytest.raises(ValueError, match="Bad commit"):
        calculate_file_commit_frequency([bad_commit], mock_repo, begin, end, top_n=20)
...
def test_calculate_file_commit_frequency_zero_stats(mock_commits_with_files, mock_repo):
